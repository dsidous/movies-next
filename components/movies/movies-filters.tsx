...
import { MOVIE_DISCOVER_SORT_OPTIONS, serializeMoviesBrowseSearchParams } from '@/lib/movies-discover-search-params';
...
const MOVIE_DISCOVER_RATING_GREATER_THAN_OPTIONS: { value: string; label: string }[] = [
...
function navigateToState(
  ...
  pathname: string,
  next: MoviesBrowseSearchState,
) {
  const qs = serializeMoviesBrowseSearchParams(next);
...
/** Same look as `Button variant="outline"` + filter row sizing + trailing chevron (matches genre trigger). */
...
export function MoviesFilters({ genres, state, className }: MoviesFiltersProps) {
  ...
  const ratingSelectValue =
    state.voteGreaterThan != null &&
    MOVIE_DISCOVER_RATING_GREATER_THAN_OPTIONS.some(
      (o) => o.value === String(state.voteGreaterThan),
    )
      ...
      <div className="flex w-full flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="grid min-w-0 w-full flex-1 gap-3 sm:grid-cols-2 lg:grid-cols-12 lg:gap-4">
          <label className="flex flex-col gap-1.5 lg:col-span-3">
            ...
            >
              {MOVIE_DISCOVER_SORT_OPTIONS.map((o) => (
          ...
          <label className="flex flex-col gap-1.5 lg:col-span-2">
            ...
            >
              {MOVIE_DISCOVER_RATING_GREATER_THAN_OPTIONS.map((o) => (
          ...
          <div className="flex flex-col gap-1.5 sm:col-span-2 lg:col-span-5">
            <span className="text-xs font-medium text-muted-foreground" id="movies-genres-label">
              Genres
            </span>
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  ...
                  aria-labelledby="movies-genres-label"
